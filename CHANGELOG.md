...
- `check` CLI now accepts `--annex-detail` to emit Annex IV technical-documentation coverage; Markdown and HTML reporters render the section pass/fail table.
...
### Added
- Completed Phase 47 CI action runtime freshness hardening:
  - upgraded CI coverage upload action to `codecov/codecov-action@v5`
  - added CI contract guard to prevent regressions to legacy `codecov/codecov-action@v3`.

### Changed
- Status surfaces synchronized to mark Phase 47 as completed across `README`, `docs/index`, and `ROADMAP`.
- Project version advanced to `0.1.38`.

### Fixed
- Reduced CI action runtime deprecation risk by removing legacy coverage upload action major usage.
...
### Added
- Completed Phase 44 maintenance v1 stabilization:
...
### Fixed
- Maintenance smoke CI contract now validates `ops_closeout_checks.json` payload shape deterministically.
- Weekly maintenance workflow now installs docs extras to keep `mkdocs --strict` execution deterministic.
...
### Added
- Completed Phase 36 GA stabilization hardening:
...
- Status surfaces synchronized to mark Phase 36 as completed (`README`, `docs/index`, `ROADMAP`).
